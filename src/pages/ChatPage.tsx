...
const Message: React.FC<MessageProps> = ({ isUser, content }) => {
  return (
    <div className={`w-full max-w-[900px] mb-4 md:mb-6 ${isUser ? 'self-end' : 'self-start'} animate-fade-in`}>
      <div className={`p-3 md:p-4 rounded-lg ${isUser ? 'bg-[#E8F3FF]' : 'bg-[#F0F9FF]'}`}>
        <p className="text-[#242424] text-sm md:text-base">{content}</p>
      </div>
    </div>
  );
};

const ChatPrompt: React.FC<{ question: string }> = ({ question }) => {
  return (
    <div className="w-full max-w-[900px] border border-gray-200 rounded-lg mb-4 md:mb-6 animate-fade-in">
      <div className="p-3 md:p-4 bg-slate-50">
        <p className="text-[#242424] text-sm md:text-base">{question}</p>
      </div>
    </div>
  );
};
...
const ChatPage: React.FC = () => {
  ...
  return (
    <div className="max-w-none flex flex-col w-full bg-white min-h-screen">
      <Header />
      <main className="flex flex-col items-center px-3 sm:px-6 md:px-12 lg:px-36 py-4 md:py-6 bg-zinc-200 rounded-none">
        <div className="flex flex-col w-full max-w-[1054px]">
          <div className="font-semibold text-lg md:text-xl text-[#242424] mb-3 md:mb-4">
            What would you like to build?
          </div>
          
          <div className="flex w-full bg-white p-2 md:p-3 rounded-[20px] mb-3 md:mb-4">
            <div className="flex items-center w-full bg-neutral-50 pl-3 md:pl-6 pr-2 md:pr-3 py-1.5 md:py-2 rounded-2xl">
              <input 
                type="text" 
                placeholder="Type your message here" 
                className="flex-1 text-sm md:text-lg text-[#242424] bg-transparent border-none outline-none" 
              ...
              <button 
                ...
                className="flex items-center justify-center rounded p-1 md:p-1.5 text-[#335CCC]"
              >
          ...
          <div className="w-full flex flex-col items-center space-y-4 md:space-y-6 py-4 md:py-8">
